...
<div class="container">
 <!-- Bouton pour générer le PDF -->
    ...
    try {
        ...
        $statement = $pdo->query("SELECT * FROM commandes WHERE semaine = 8 ORDER BY date");
...
<h1 style="color: red;">LISTE DES MASAKA</h1>
...
<h1 style="color: blue;">LISTE DES VILLES</h1>
...
foreach ($rows as $row) {
    ...
    // Add values to the corresponding keys in $ListTown based on CP
    if (!isset($ListTown[$cp])) {
        $ListTown[$cp] = [];
    }
...
// Iterate through each CP group in $ListTown
foreach ($ListTown as $cp => $towns) {
    // Output the CP
    echo "CODE POSTAL = $cp<br>";

    // Iterate through each town in the CP group
    foreach ($towns as $index => $values) {
        // Output the index (1, 2, 3, ...)
        echo ($index + 1) . ') ';

        // Output the values for 'adresse', 'cp', and 'ville'
        echo $values['adresse'] . ', ' . $values['cp'] . ', ' . $values['ville'] . '<br>';
    }

    // Add a line break between CP groups
    echo '<br>';
}
?>
